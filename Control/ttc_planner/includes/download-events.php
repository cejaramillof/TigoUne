...
$regional   = '';
$resultUser = $wpdb->get_row( "SELECT * FROM ttr_control_structure WHERE id = '$userID'", ARRAY_A );
...
$planner      = new Planner();
$resultEvents = $planner->get_events( $year, $month, $userID );
...
/*$objDrawing      = new PHPExcel_Worksheet_Drawing();
$outsourcingLogo = "../assets/logo_outsourcing.png";
$objDrawing->setPath( $outsourcingLogo );
...
$objPHPExcel->getActiveSheet()->getStyle( 'A1:J7' )->getFill()->setFillType( PHPExcel_Style_Fill::FILL_SOLID )->getStartColor()->setARGB( 'FFFFFF' );


$objPHPExcel->getActiveSheet()->getStyle( 'B3' )->applyFromArray( $styleSubtitle );
$objPHPExcel->getActiveSheet()->getStyle( 'B8:J8' )->applyFromArray( $styleTitle );
...
$objPHPExcel->setActiveSheetIndex( 0 )
            ...
            ->setCellValue( 'D4', $resultUser['user_regional'] )
            ->setCellValue( 'B5', 'Cédula del Entrenador:' )
            ->setCellValue( 'D5', $resultUser['id'] )
            ->setCellValue( 'B6', 'Nombre del Entrenador:' )
            ->setCellValue( 'D6', $resultUser['user_name'] )
            ->setCellValue( 'B8', 'Fecha' )
            ->setCellValue( 'C8', 'Regional' )
            ->setCellValue( 'D8', 'Canal' )
            ->setCellValue( 'E8', 'Código' )
            ->setCellValue( 'F8', 'Nombre del punto' )
            ->setCellValue( 'G8', 'Actividad' )
            ->setCellValue( 'H8', 'Observaciones' )
            ->setCellValue( 'I8', 'Departamento' )
            ->setCellValue( 'J8', 'Ciudad' );
...
if ( $resultEvents == false ) {
	...
	foreach ( $resultEvents as $event ) {
		$objPHPExcel->setActiveSheetIndex( 0 )
		            ...
		            ->setCellValue( 'C' . $i, $event['regional'] )
		            ->setCellValue( 'D' . $i, $event['channel'] )
		            ...
		            ->setCellValue( 'I' . $i, $event['department'] )
		            ->setCellValue( 'J' . $i, $event['city'] );


		$objPHPExcel->getActiveSheet()->getStyle( 'B' . $i )->getNumberFormat()->setFormatCode( 'dd/mm/yyyy' );
		$objPHPExcel->getActiveSheet()->getStyle( 'A' . $i . ':J' . $i )->getFill()->setFillType( PHPExcel_Style_Fill::FILL_SOLID )->getStartColor()->setARGB( 'FFFFFF' );
		$objPHPExcel->getActiveSheet()->getStyle( 'B' . $i . ':J' . $i )->applyFromArray( array(
			'borders' => array(
				'bottom' => array(

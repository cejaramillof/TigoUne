...
(SELECT user_regional, id, user_name, user_role
 ...
 WHERE user_role NOT IN ('MANAGER', 'UBERMANAGER') AND to_stats = 1 AND estado = 'ACTIVO') AS structure
	...
	<table id="control-trainer-results-trainer" class="control-trainer-results">
		...
		foreach ( $result2 as $trainer ) { ?>
			...
			<tr>
				...
				<td><a href="?page=reports&trainerView&trainerID=<?php echo $trainer->usuario; ?>&date=<?php echo $month; ?>"><?php echo ucwords(strtolower($trainer->nombre)); ?></a></td>

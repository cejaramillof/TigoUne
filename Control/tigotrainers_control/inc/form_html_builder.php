...
function gen_unique_option($item) {
    ...
    foreach ($item->options as $option)
    {
        ...
        $title_movil = explode( '[MOVIL]',  htmlentities($option->option_key) );
        if( isset( $titlemovil[1] ) && $status_title_movil ):
            $html .= '<h4 style="margin:10px 0;padding:0;">Entrenamiento Movil</h4>';
            $status_title_movil = false;
        endif;
...
function gen_multiple_option($item) {
    ...
    foreach ($item->options as $option) {
        ...
        $titulo_servicios2 = explode( '[SERVICIO CONTACT]',  htmlentities($option->option_key) );
        if( isset( $titulo_servicios2[1] ) && $status_title_servicios2 ):
            $html .= '<h4 style="margin:10px 0;padding:0;">MATRIZ SERVICIO CONTACT</h4>';
            $status_title_servicios2 = false;
        endif;

        $titulo_servicios = explode( '[SERVICIO TIENDA]',  htmlentities($option->option_key) );
        if( isset( $titulo_servicios[1] ) && $status_title_servicios ):
            $html .= '<h4 style="margin:10px 0;padding:0;">MATRIZ SERVICIO TIENDA</h4>';
        ...
        if( isset( $titulo_otros[1] ) && $status_title_otros ):
            $html .= '<h4 style="margin:10px 0;padding:0;">OTROS MATRIZ</h4>';
        ...
        $titulo_movil = explode( '[MOVIL]',  htmlentities($option->option_key) );
        if( isset( $titulo_movil[1] ) && $status_title_movil ):
            $html .= '<h4 style="margin:10px 0;padding:0;">MATRIZ MOVIL</h4>';
            $status_title_movil = false;
        endif;

        $titulo_hogares = explode( '[HOGARES]',  htmlentities($option->option_key) );
        if( isset( $titulo_hogares[1] ) && $status_title_hogares ):
            $html .= '<h4 style="margin:10px 0;padding:0;">MATRIZ HOGARES</h4>';
            $status_title_hogares = false;
        endif;

        if( !isset( $titulo_otros[1] ) && !isset( $titulo_servicios[1] ) && !isset( $titulo_otros_servicios[1] ) && !isset( $titulo_convergencia_retail[1] ) && !isset( $titulo_hogares[1] ) && !isset( $titulo_movil[1] ) && $status_title_comercial ):

...
class FormulariosController extends Controller
{
	...
	############################################################################# HISTORICO Y DESCARGA ACOMPAÑAMIENTOS
		...
		############################################################################# CONSULTAS SEGÚN ROL
			private static function getQueryHistoricoAcompanamientos( $tipo_formulario = 1, $desde = '0000-00-00', $hasta = '0000-00-00' )
			{
				...
				if( Auth::user() -> jerarquia == 'ENTRENADOR_COMERCIAL' || Auth::user() -> jerarquia == 'ENTRENADOR_SERVICIO' || Auth::user() -> jerarquia == 'ENTRENADOR_TECNICOS' || Auth::user() -> jerarquia == 'SUPERVISOR_COMERCIAL' || Auth::user() -> jerarquia == 'SUPERVISOR_TECNICOS'):
					...
					$data_a = $data_a -> whereIN( 'formularios.id', [1, 2, 3, 4, 5, 6, 7, 8, 9, 13, 12, 15, 16, 19] );
					...
					$data_b = $data_b -> whereIN( 'formularios.id', [1, 2, 3, 4, 5, 6, 7, 8, 9, 13, 12, 15, 16, 19] );
				...
				elseif( Auth::user() -> jerarquia == 'ENTRENADOR_COMERCIAL_LIDER' || Auth::user() -> jerarquia == 'ESPECIALISTA_REGIONAL' || Auth::user() -> jerarquia == 'COORDINADOR_TECNICOS' || Auth::user() -> jerarquia == 'ESPECIALISTA_SERVICIO'):
                    if ($tipo_formulario != 1):
				        $data_a = $data_a -> where( 'formularios.id_tipo_formulario', '=', $tipo_formulario );
                    endif;
					...
					$data_a = $data_a -> whereIN( 'formularios.id', [1,2,3, 4, 5, 6, 7, 8, 9, 12, 13, 15, 16, 19] );
					$data_a = $data_a -> groupBY( 'formulario.id' );
					$data_a = $data_a -> orderBY( 'formularios.id' );
					$data_a = $data_a -> get();
					...
					$data_b = $data_b -> whereIN( 'formularios.id', [1,2,3, 4, 5, 6, 7, 8, 9, 12, 13, 15, 16, 19] );
					$data_b = $data_b -> groupBY( 'formulario.id' );
					$data_b = $data_b -> orderBY( 'formularios.id' );
					$data_b = $data_b -> get();
		...
		############################################################################# CREA Y DESCARGA ARCHIVO .csv SEGÚN CONSULTA
			public static function getDescargarAcompanamientos( Request $solicitud )
			{
				...
				if( Auth::user() -> jerarquia == 'ADMINISTRADOR' || Auth::user() -> jerarquia == 'OUTSOURCING' ):
                  ...
                  endforeach;            
				if( Auth::user() -> jerarquia != 'ADMINISTRADOR' || Auth::user() -> jerarquia != 'OUTSOURCING' ):
					array_push($headers['headers'], 'OBSERVACIONES');
				endif;
                ...
                if( Auth::user() -> jerarquia == 'OUTSOURCING' ):
                  array_push($headers['headers'], 'RESULTADO');
                endif;
				foreach( $query AS $c ):
                    ...
                    if( Auth::user() -> jerarquia == 'ADMINISTRADOR' || Auth::user() -> jerarquia == 'OUTSOURCING' ):
                      ...
                      endforeach;
										...
										if( Auth::user() -> jerarquia != 'ADMINISTRADOR' || Auth::user() -> jerarquia != 'OUTSOURCING' ):
											if( !isset( $rpta_pregunta[ 'Observaciones generales-'.$c -> ID ] ) ):
													array_push( $valores[$c -> ID], '' );
											else:
													array_push( $valores[$c -> ID], $rpta_pregunta[ 'Observaciones generales-'.$c -> ID ] );
											endif;
										endif;
                    ...
                    if( Auth::user() -> jerarquia == 'OUTSOURCING' ):
                      if( $rpta_aspectos[ 'NOTA PONDERADA-'.$c -> ID ] >= 3 ):
                          array_push( $valores[$c -> ID], 'APROBADO' );
                      else:
                          array_push( $valores[$c -> ID], 'REPROBADO' );
                      endif;
                    endif;
		...
		############################################################################# CREA JSON PARA CARGA EN DATATABLE
			public static function getHistoricoAcompanamientos( Request $solicitud )
			{
				...
				if( count($query) > 0 ):
					//  'AÑO','MES','DÍA','HORA',
					$json['headers'] = [ 'ID','FORMULARIO','GRUPO', 'FECHA_SISTEMA', 'CEDULA_EVALUADOR', 'NOMBRE_EVALUADOR', 'CARGO_EVALUADOR', 'REGIONAL_EVALUADOR', 'CEDULA_EVALUADO', 'NOMBRE_EVALUADO', 'CARGO_EVALUADO', 'PROVEEDOR_EVALUADO', 'ESTRATEGIA_EVALUADO', 'REGIONAL_EVALUADO', 'PLAZA_EVALUADO', 'CANAL_EVALUADO', 'FECHA_ACTIVIDAD' ];

					$json['data'] = [];
					foreach( $query AS $c ):
						//, $c -> ANIO, $c -> MES, $c -> DIA, $c -> HORA
						array_push( $json['data'], [ $c -> ID, $c -> FORMULARIO, $c -> GRUPO, $c -> FECHA_SISTEMA, $c -> CEDULA_EVALUADOR, $c -> NOMBRE_EVALUADOR, $c -> CARGO_EVALUADOR, $c -> REGIONAL_EVALUADOR, $c -> CEDULA_EVALUADO, $c -> NOMBRE_EVALUADO, $c -> CARGO_EVALUADO, $c -> PROVEEDOR_EVALUADO, $c -> ESTRATEGIA_EVALUADO, $c -> REGIONAL_EVALUADO, $c -> CIUDAD_EVALUADO, $c -> CANAL_EVALUADO, $c -> FECHA_ACTIVIDAD ] );
	...
	public static function getCedula( Request $solicitud )
	{
		if( $solicitud -> ajax() ):
            ...
            if( count($data) <= 0 ):
              $data = DB::table('users_ext')
                      -> select( 'cedula', 'nombre_empleado', 'empresa AS cargo', 'regional', 'canal' )
                      -> where ( 'cedula', 'like', '%'.$solicitud.'%')
                      -> get();
            endif;
